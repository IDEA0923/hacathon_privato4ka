...
_LEVEL_RE = re.compile(r"\b(III|II|I)\s+уровень\b", re.IGNORECASE)
...
def _parse_level(text: str) -> str:
    """Извлекает уровень олимпиады из текста (I/II/III уровень)."""
    if not text:
        return ""
    m = _LEVEL_RE.search(text)
    if m:
        return f"{m.group(1)} уровень"
    return ""
...
class OlimpiadaRu(BaseSource):
    ...
    async def fetch(
        ...
        for a in tree.css("a"):
            ...
            level = _parse_level(ctx_text) or _parse_level(title)
