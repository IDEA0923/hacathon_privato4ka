...
class Event:
    ...
    name: str                   # до 255 симв.
    date_start: datetime
    date_end: Optional[datetime]
    class_start: int
    class_end: int
    level: str                  # до 32 симв.
    source: str                 # до 64 симв. (frm)
    url: str                    # полный URL (в БД пойдёт md5)
    subjects: str               # до 32 симв., коды
    description: str            # до 1000 симв.
    ...
    async def insert_event(self, ev: Event) -> bool:
        ...
        async with self._pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                _clip(ev.source, 64),
                h,
                _clip(ev.subjects, 32),
                _clip(ev.description, 1000),
            )
            return True
