...
from keyboards.onboarding import consent_keyboard, main_menu_keyboard, open_webapp_keyboard
...
async def handle_start(
    ...
    profile = await user_service.get_profile(tg_user.id)
    if (
        profile is not None
        and profile.subjects is not None
        and profile.class_ is not None
        and profile.region is not None
    ):
        await state.clear()
